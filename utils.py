...
def filter_nft_adventure(contract, nft_set, w3):
    height = w3.eth.get_block('latest').number

    def _thread(contract, id, height, nft_set):
        #print(id)
        if height <= contract.functions.adventurers_log(int(id)).call():
            nft_set.remove(id)

    threads = []

    for id in list(nft_set):
        t = threading.Thread(target=_thread, args=(contract, id, height, nft_set))
        t.start()
        threads.append(t)
    for t in threads:
        t.join()
